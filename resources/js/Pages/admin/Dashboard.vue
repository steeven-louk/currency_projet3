...
const showModal = ref(false);//affichage du modal permettant l'ajout d'une paire
const showUpdateModal = ref(false);//affichage du modal permettant l'update d'une devise
const showPaireModal = ref(false); //affichage du modal permettant l'ajoutd'une paire
const showPaireUpdateModal = ref(false); //affichage du modal permettant l'update d'une paire
...
const editData = {}; //variable recevant les donnée pour mise à jour
        ...
        <div class="py-12 relative">
            ...
            <div class="modal" v-show="showModal">
                <button type="button" @click="showModal = false"
                    class="font-bold ml-auto p-2 bg-red-500 rounded block cursor-pointer">X</button>
            ...
            <div class="modal" v-show="showPaireModal">
                <button type="button" @click="showPaireModal = false"
                    class="font-bold ml-auto p-2 bg-red-500 rounded block cursor-pointer">X</button>
                <form @submit.prevent="addPaire" class="flex flex-col gap-4 mx-auto align-center justify-center">
                    ...
                    <button type="submit"
                        class="bg-green-800 text-white font-bold uppercase mx-auto w-[15em] p-2 mt-3 rounded-md">add
                        paire</button>
            ...
            <div class="modal" v-if="showUpdateModal">
                ...
                <form @submit.prevent="updateDevise(editData.value.id)"
            ...
            <div class="modal" v-if="showPaireUpdateModal">
                <button type="button" @click="showPaireUpdateModal = false"
                    class="font-bold ml-auto p-2 bg-red-500 rounded block cursor-pointer">X</button>
                <form @submit.prevent="updatePaire(editData.value.id)"
                    class="flex flex-col gap-4 mx-auto align-center justify-center">

                    <input type="text" max="3" placeholder="Devise 1" required class="form-input"
                        v-model="editData.devise_1" name="devise_1">
                    <input type="text" max="3" placeholder="Devise 2" required class="form-input"
                        v-model="editData.devise_2" name="devise_2">
                    <input type="number" step="0.01" placeholder="Taux" required class="form-input" v-model="editData.taux"
                        name="taux">
                    <button type="submit"
                        class="bg-green-800 text-white font-bold uppercase mx-auto w-[15em] p-2 mt-3 rounded-md">update</button>
                ...
                <div class="overflow-hidden flex flex-wrap align-middle justify-center gap-3 shadow-sm sm:rounded-lg">
                    ...
                    <div class="p-6 mt-12   bg-white border-b w-[100%] max-w-2xl border-gray-500">
                        <div class="header flex mb-5">
                            ...
                            <button @click="showPaireModal = true"
                                class="bg-green-800 text-white font-bold uppercase ml-auto w-[15em] p-2 rounded-md">add
                                paires</button>
                        </div>
                        <hr>
